...
export type ElevationBand = "lowland" | "hill" | "mountain";

const HILL_THRESHOLD = 0.33;
const MOUNTAIN_THRESHOLD = 0.7;

/**
 * Which discrete elevation band a raw elevation value falls into, normalized against the run's
 * own terrainRoughness (the parameter that actually controls how tall generated terrain gets) —
 * not the hard [0,3] god-mode clamp. That keeps the three bands meaningful whether the map is a
 * gentle default hill-scape or a player has hand-raised a mountain far above what natural
 * generation produces: a hand-raised peak past terrainRoughness just clamps into "mountain"
 * rather than needing a fourth category.
 */
export function elevationBand(elevation: number, terrainRoughness: number): ElevationBand {
  const norm = clamp01(elevation / Math.max(terrainRoughness, 1e-6));
 ...
 * elevation), so without this a hand-drawn barrier would be invisible on the map.
 */
export function terrainCellColor(elevation: number, fertility: number, passability: number, terrainRoughness: number): string {
  const roughness = Math.max(terrainRoughness, 1e-6);
  const norm = clamp01(elevation / roughness);
  const band = elevationBand(elevation, roughness);
